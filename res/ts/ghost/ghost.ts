...
export class Ghost extends GhostTemplate {
  ...
  private findPathToPacman(): { posX: number; posY: number }[] | null {
    const start = {
      // Pozice ducha
    ...
    const closedList: Set<string> = new Set(); // v této sadě jsou uloženy cesty, které duch již prozkoumal
    const cameFrom = new Map<string, { posX: number; posY: number }>(); // uchovává pozice, odkud duch přišel
    const gScore: Map<string, number> = new Map<string, number>(); // uchovává skutečnou vzdálenost
    const fScore: Map<string, number> = new Map<string, number>(); // uchovává odhadovanou vzdálenost (cenu / výhodnost té cesty)
    ...
    while (openList.length > 0) {
      const current: { posX: number; posY: number } = openList.shift()!; // odebere první uzel z openList a nastaví ho jako aktuální
      openList.sort((a, b) => {
        ...
        if (fScoreA === fScoreB) {
          ...
          if (dirA === currentDir) return -1;
          if (dirB === currentDir) return 1;
        }
      ...
      if (
        ...
        let step: { posX: number; posY: number } | undefined = current; // od cílové pozice zpětně rekonstruuje cestu do počáteční pozice
        while (step) {
          path.unshift(step);
      ...
      for (const direction of [
        ...
        if (
          ...
          const tentativeGScore =
            (gScore.get(`${current.posX},${current.posY}`) ?? Infinity) + 1;
          ...
          if (
            ...
            fScore.set(
              `${nx},${ny}`,
              tentativeGScore + this.heuristic({ posX: nx, posY: ny }, target)
            );
            cameFrom.set(`${nx},${ny}`, current);
            // pokud nový uzel ještě není v openList, přidá ho
            if (
              !openList.some((node) => node.posX === nx && node.posY === ny)
            ) {
     ...
     */
    if (this.mode === "frightened") {
      let farthestPath: { posX: number; posY: number }[] = [];
      ...
      for (const point of closedList) {
        ...
        if (dist > maxDistance) {
          maxDistance = dist;
          farthestPath = [];
          let step: { posX: number; posY: number } | undefined = {
            posX: px,
            posY: py,
          };
          while (step) {
  ...
  private getDirectionBetweenNodes(
    from: { posX: number; posY: number },
    to: { posX: number; posY: number }
  ): string {
    if (to.posX > from.posX) return "right";
    if (to.posX < from.posX) return "left";
    if (to.posY > from.posY) return "down";
    return "up";
  }
